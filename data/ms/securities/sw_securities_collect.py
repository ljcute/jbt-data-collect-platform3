    ...
    def target_collect(cls):
        ...
        try:
            ...
            if df_result is not None:
                ...
                raise Exception(f'采集数据条数{int(len(original_data_list))}，与官网条数{sc_total}不一致，采集失败')

            message = "申万宏源证券融资融券标的证券数据采集完成"
    ...
    def guaranty_collect(cls):
        ...
        try:
            ...
            if df_result is not None:
                ...
                raise Exception(f'采集数据条数{int(len(all_data_list))}，与官网条数{sc_total}不一致，采集失败')

            message = "申万宏源证券融资融券担保券数据采集完成"
            super().kafka_mq_producer(json.dumps(actual_date, cls=ComplexEncoder),
